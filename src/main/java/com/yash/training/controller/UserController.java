...
import com.yash.training.beans.LoginCommand;
import com.yash.training.beans.User;
import com.yash.training.service.impl.MapValidationErrorService;
import com.yash.training.service.impl.UserServiceImpl;
...
public class UserController {
	...
	public ResponseEntity<?> getUserById(@PathVariable Integer id) {
		Optional<User> user = userServiceImpl.findByUId(id);
    ...
    @PostMapping("/login")
	public ResponseEntity<?> signUp(@Valid @RequestBody LoginCommand loginCommand, BindingResult result,HttpSession session){
    	
		ResponseEntity<?> errorMap = mapValidationErrorService.mapValidationError(result);
		if (errorMap != null)
			return errorMap;
		Optional<User> loggedinuser = userServiceImpl.authenticate(loginCommand.getUserId(), loginCommand.getPassword());
		
		if (loggedinuser != null){
			setLoggedInUserInSession(session, loggedinuser);
			return new ResponseEntity<Optional<User>>(loggedinuser,HttpStatus.OK);
		} 
		else{
...
/*	
	...
	private void setLoggedInUserInSession(HttpSession session, Optional<User> loggedinuser) {
